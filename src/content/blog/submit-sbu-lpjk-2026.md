---
title: "Proses Submit SBU ke LPJK 2026: Panduan Lengkap"
description: "Step-by-step proses pengajuan SBU ke LPJK 2026 — dari persiapan akun hingga terbit sertifikat. Hindari error umum yang bikin proses molor berhari-hari."
channel: blog
product: SBU-M
image: /blog/submit-sbu-lpjk-2026.webp
voice_mode: corporate
publishedAt: 2026-04-26
dateModified: 2026-04-26
faqSchema:
  - q: "Bagaimana cara submit SBU ke LPJK 2026?"
    a: "Proses submit SBU ke LPJK dilakukan melalui portal SIKI LPJK di siki.lpjk.net. Langkahnya: daftarkan akun badan usaha, isi data profil perusahaan, unggah dokumen sesuai kualifikasi K/M/B, pilih sub-bidang yang diajukan, kirim pengajuan, dan pantau status verifikasi. Seluruh proses dilakukan secara online — tidak ada kunjungan fisik ke kantor LPJK. Berdasarkan 280+ kasus yang Nicx proses, submit yang sudah disiapkan matang biasanya masuk proses verifikasi dalam 1-2 hari kerja."
  - q: "Berapa lama dari submit hingga SBU terbit?"
    a: "Dari submit hingga SBU terbit: SBU Kecil 3-7 hari kerja, SBU Menengah 14-21 hari kerja, SBU Besar 21-30 hari kerja. Hitungan ini dimulai setelah dokumen dinyatakan lengkap oleh LPJK, bukan sejak upload pertama kali. Kalau ada revisi dokumen yang diminta, timeline otomatis bergeser. Nicx telah mengurus 280+ sertifikat dan waktu tercepat SBU Kecil yang pernah Nicx proses adalah 3 hari kerja dengan dokumen 100% lengkap sejak hari pertama."
  - q: "Apa yang terjadi kalau dokumen submit SBU ditolak LPJK?"
    a: "Kalau LPJK meminta revisi atau menemukan ketidaksesuaian, pengajuan akan dikembalikan dengan catatan perbaikan. Perusahaan Anda harus memperbaiki dokumen sesuai catatan, lalu mengunggah ulang. Setiap siklus revisi rata-rata menambah 3-7 hari kerja ke timeline. Untuk menghindari ini, pastikan checklist dokumen sudah selesai 100% sebelum submit, dan KSWP serta KBLI sudah beres lebih dulu."
---
## TL;DR

Submit SBU ke LPJK dilakukan via portal SIKI di siki.lpjk.net. Proses online penuh — tidak perlu ke kantor LPJK. Dari submit hingga terbit: **3-7 hari kerja** (SBU K), **14-21 hari** (SBU M), **21-30 hari** (SBU B). Dokumen tidak lengkap = proses ulang dari awal.

Untuk verifikasi status terbaru, Anda bisa langsung cek [LPJK Online](https://lpjk.pu.go.id).

Pastikan data perusahaan Anda sinkron di [OSS RBA](https://oss.go.id) sebelum submit.

## Submit SBU Dilakukan di Mana?

![Contoh SBU yang berhasil diterbitkan Atrahdis](/blog/cert-submit-sbu-lpjk-2026.jpg)

Sejak digitalisasi LPJK, seluruh proses pengajuan SBU dilakukan melalui portal SIKI LPJK (Sistem Informasi Konstruksi Indonesia) di **siki.lpjk.net**.

Tidak ada submit fisik ke kantor LPJK pusat atau daerah. Semua dokumen diunggah secara digital, verifikasi dilakukan online, dan SBU terbit dalam format digital yang bisa diunduh.

Ini berarti Anda bisa mengurus SBU dari mana saja, tetapi juga berarti tidak ada "jalan pintas" via pendekatan personal ke petugas lokal. Dokumen harus benar dari awal.

## Langkah 1: Siapkan Akun SIKI LPJK

Sebelum submit, pastikan akun badan usaha Anda di SIKI aktif:

1. Buka siki.lpjk.net
2. Pilih "Registrasi Badan Usaha" kalau belum punya akun
3. Input data: NIB, NPWP, nama perusahaan, data PIC
4. Verifikasi email yang didaftarkan
5. Login dan lengkapi profil perusahaan

Akun yang sudah pernah dipakai untuk pengajuan sebelumnya bisa langsung digunakan. Pastikan data profil masih akurat — nama, alamat, NPWP, dan data pengurus.

## Langkah 2: Isi Data Pengajuan

Setelah login:

1. **Pilih menu "Pengajuan SBU"**
2. **Pilih kualifikasi** — Kecil, Menengah, atau Besar
3. **Pilih sub-bidang** — pastikan ini cocok dengan KBLI yang aktif di NIB Anda
4. **Isi data badan usaha** — modal, pengurus, tenaga ahli
5. **Input data SKK** — NIK dan nomor SKK tenaga ahli yang ditunjuk
6. **Simpan draft** sebelum unggah dokumen

Jangan langsung klik "Submit" setelah isi data. Simpan dulu sebagai draft, verifikasi semua data, baru submit.

## Langkah 3: Unggah Dokumen

Urutan unggah yang disarankan:

| Urutan | Dokumen | Format | Ukuran Maks |
|--------|---------|--------|-------------|
| 1 | NIB | PDF | 2 MB |
| 2 | Akta pendirian + perubahan | PDF | 5 MB |
| 3 | SK Kemenkumham | PDF | 2 MB |
| 4 | NPWP badan usaha | PDF | 1 MB |
| 5 | KSWP (status valid) | PDF/screenshot | 1 MB |
| 6 | KTP direktur/pengurus | PDF/JPG | 1 MB |
| 7 | SKK tenaga ahli | PDF | 2 MB |
| 8 | Surat pernyataan kebenaran | PDF | 1 MB |
| 9 | Dokumen tambahan (M/B) | PDF | sesuai jenis |

Pastikan semua file terbaca jelas — scan buram atau file rusak adalah penyebab revisi yang paling mudah dihindari.

## Langkah 4: Verifikasi dan Submit

Sebelum klik tombol submit final:

1. Baca ulang semua data yang sudah diinput
2. Pastikan sub-bidang yang dipilih sesuai KBLI
3. Verifikasi SKK tenaga ahli sudah benar level dan bidangnya
4. Cek semua dokumen sudah terunggah dan terbaca
5. Klik "Submit Pengajuan"

Setelah submit, Anda akan mendapat nomor registrasi pengajuan. Simpan nomor ini untuk memantau status.

## Langkah 5: Pantau Status Pengajuan

Status pengajuan bisa dipantau di SIKI LPJK dengan nomor registrasi:

| Status | Artinya | Yang Harus Dilakukan |
|--------|---------|----------------------|
| Diterima | Dokumen masuk, menunggu verifikasi | Tunggu |
| Dalam Verifikasi | Sedang diperiksa tim LPJK | Tunggu, siapkan dokumen tambahan jika diminta |
| Revisi | Ada yang perlu diperbaiki | Baca catatan revisi, perbaiki, upload ulang |
| Disetujui | Proses selesai, SBU akan terbit | Unduh SBU setelah terbit |
| Ditolak | Pengajuan tidak memenuhi syarat | Identifikasi alasan, perbaiki dari awal |

## Apa yang Sering Membuat Submit Harus Revisi?

Dari pengalaman 280+ kasus, lima penyebab revisi paling umum:

1. **KSWP tidak valid saat submit** — cek dulu sebelum upload
2. **SKK level tidak sesuai** — baca Perlem LPJK terbaru untuk persyaratan level
3. **Dokumen scan buram** — gunakan scanner atau kamera dengan resolusi cukup
4. **Sub-bidang tidak cocok KBLI** — verifikasi kecocokan di OSS
5. **Surat pernyataan tidak ditandatangani asli** — harus tanda tangan basah atau elektronik resmi

Untuk checklist dokumen lengkap per kualifikasi, baca [artikel dokumen SBU konstruksi checklist](/blog/dokumen-sbu-konstruksi-checklist). Untuk memahami penyebab penolakan SBU lebih dalam, baca [8 alasan SBU ditolak dan solusinya](/blog/kenapa-sbu-ditolak-solusi). Kalau Anda ingin proses yang lebih cepat tanpa revisi bolak-balik, cek layanan [SBU Kecil](/layanan/sbu-kecil) atau [SBU Menengah](/layanan/sbu-menengah).

---

*Ditulis oleh **Nicx** — SBU Specialist, PT Atrahdis (sejak 2016, 280+ sertifikat)*


## Kesimpulan

Submit SBU ke LPJK di 2026 mengikuti alur yang lebih terintegrasi dengan OSS. Anda perlu memastikan semua dokumen dasar sudah benar sebelum submit, karena sistem akan memverifikasi data Anda secara otomatis. Satu ketidaksesuaian bisa menghentikan proses.

Jangan terburu-buru submit tanpa audit dokumen terlebih dahulu. Waktu yang Anda habiskan untuk pengecekan awal akan menghemat waktu berminggu-minggu di kemudian hari.